...
class PlotPanel extends Panel
{
    ...
    void drawAxes()
    {
        ...
        // X axis ticks.
        for (int i = (int) Math.round(xMin); i <= (int) Math.round(xMax); ++i) {
            int c = (int) Math.round((i - xMin)/xDelta);
        ...
        // Y axis ticks.
        for (int i = (int) Math.round(yMin); i <= Math.round(yMax); ++i) {
            int r = (int) Math.round((yMax - i)/yDelta);
    ...
    void drawXequalsY()
    {
        ...
        double p1 = Math.max(xMin, yMin);
        int   c1 = (int) Math.round((p1 - xMin)/xDelta);
        int   r1 = (int) Math.round((yMax - p1)/yDelta);
        double p2 = Math.min(xMax, yMax);
        int   c2 = (int) Math.round((p2 - xMin)/xDelta);
        int   r2 = (int) Math.round((yMax - p2)/yDelta);
     ...
     */
    void plot(int c, double y)
    {
        if (bg == null) return;

        // Plot y if it's within range.
        if ((y >= yMin) && (y <= yMax)) {
            int r = (int) Math.round((yMax - y)/yDelta);
            cs[k] = c;
            rs[k] = r;
            ++k;
        }
