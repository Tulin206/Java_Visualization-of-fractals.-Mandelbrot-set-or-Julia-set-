                                 ...
                                 implements DemoPanel
{
    ...
    /** plot bounds */                  private double xMin, xMax, yMin, yMax;
    ...
    protected void plotFunction()
    {
        ...
        double xMin   = plotProps.getXMin();
        double deltaX = plotProps.getXDelta();

        // Loop over each pixel of the x axis to plot the function value.
        for (int c = 0; c < w; ++c) {
            double x = xMin + c*deltaX;
            double y = valueAt(x);
     ...
     */
    protected void checkFunctionInBounds(double a, double b) throws Exception
    {
        // Plot properties.
        int   w      = plotProps.getWidth();
        double xMin   = plotProps.getXMin();
        double yMin   = plotProps.getYMin();
        double yMax   = plotProps.getYMax();
        double deltaX = plotProps.getXDelta();

        int ca = (int) Math.round((a - xMin)/deltaX);
        int cb = (int) Math.round((b - xMin)/deltaX);
     ...
     */
    protected double valueAt(double x) { return 0; }
     ...
     */
    protected void plot(int c, double y) { plotPanel.plot(c, y); }
